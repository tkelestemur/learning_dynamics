...
class LSTMAutoEncoder(nn.Module):
    ...
    def __init__(self, input_size, action_size, hidden_size, num_layers, bias=False, future_steps=1, checkpoint_path=None,
                 loss_path=None):
        ...
        for epoch in range(num_epochs):
            ...
            for states, actions in train_data_loader:
                ...
                if self.future_steps == 1:
                    ...
                    rec_loss = loss_func(input=decoded, target=states)
                    pred_loss = loss_func(input=transformed[:, :-1, :], target=encoded[:, 1:, :])
                    ...
                    rec_loss = loss_func(input=decoded, target=states)
                    pred_loss = loss_func(input=transformed[:, :-1, :], target=encoded[:, 1:, :]) + loss_func(
                        input=transformed_2[:, :-1, :], target=encoded[:, 2:, :])
                else:
